...
import { MessageSquare, Send, PawPrint } from 'lucide-react';
...
export default function ChatPage() {
  ...
  return (
    <div className="flex h-full">
      {/* Sidebar */}
      <div className="w-72 border-r border-navy-100 bg-white flex flex-col">
          ...
          {conversations.map(c => {
            ...
            return (
              <button
                ...
                className={`w-full text-left p-4 border-b border-navy-50 hover:bg-navy-50 transition-colors ${Number(tutorId) === c.id ? 'bg-teal-50 border-l-2 border-l-teal-400' : ''}`}
              >
      ...
      {!tutorId ? (
        ...
        <div className="flex-1 flex flex-col">
          {/* Header */}
          <div className="px-6 py-4 border-b border-navy-100 bg-white">
            ...
            {messages.map(msg => {
              ...
              return (
                <div key={msg.id} className={`flex ${isMine ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-xs lg:max-w-md px-4 py-2.5 rounded-2xl text-sm ${isMine ? 'bg-navy-700 text-white rounded-br-sm' : 'bg-white border border-navy-100 text-navy-700 rounded-bl-sm shadow-sm'}`}>
                    <p>{msg.body}</p>
