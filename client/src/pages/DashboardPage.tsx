...
export default function DashboardPage() {
  ...
  return (
    <div className="p-8">
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-navy-700">Bienvenida, {user?.name} 👋</h1>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        ...
        <StatCard icon={TrendingUp} color="navy" label="Citas esta semana" value={s?.weekAppointments ?? 0} />
      </div>
            ...
            {data?.recentPatients.map(p => (
              <Link key={p.id} to={`/patients/${p.id}`} className="flex items-center gap-3 p-3 rounded-xl hover:bg-navy-50 transition-colors group">
                ...
                <div className="min-w-0">
                  <div className="font-medium text-navy-700 group-hover:text-teal-600 truncate">{p.name}</div>
                  <div className="text-xs text-navy-400 truncate">{p.species}{p.breed ? ` · ${p.breed}` : ''} · {p.tutor.name}</div>
                </div>
                <div className="text-xs text-navy-300 ml-auto flex-shrink-0">
          ...
          <div className="space-y-3">
            ...
            {data?.upcomingAppointments.map(a => (
              <div key={a.id} className="flex items-center gap-3 p-3 rounded-xl bg-navy-50">
                ...
                <div className="text-xs text-navy-500 ml-auto flex-shrink-0 text-right">
...
function StatCard({ icon: Icon, color, label, value, sub }: { icon: React.ElementType; color: 'teal' | 'navy'; label: string; value: number; sub?: string }) {
  return (
    <div className="card flex items-center gap-4">
      <div className={`w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0 ${color === 'teal' ? 'bg-teal-100' : 'bg-navy-100'}`}>
